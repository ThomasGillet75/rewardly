...
class FirestoreFriendlyService {
  ...
  Future<void> addFriend(Friendly friend) async {
    ...
    final Friendly friend2 =
        Friendly(friendId: friend.userId, userId: friend.friendId);
  ...
  Future<List<Users>> getUser() async {
    ...
    List<Users> users = userSnapshot.docs
        .map((doc) => Users.fromMap(doc.data() as Map<String, dynamic>))
        .toList();
    List<Friendly> friends = friendSnapshot.docs
        .map((doc) => Friendly.fromMap(doc.data() as Map<String, dynamic>))
        .toList();

    return users
        .where(
            (element) => friends.any((friend) => friend.friendId == element.id))
        .toList();
  }
  ...
  Future<void> removeFriend(Friendly friendly) async {
    ...
    await _firestore
        .collection('friendly')
        .where('user_id', isEqualTo: friendly.friendId)
        .where('friend_id', isEqualTo: _firebaseAuth.currentUser!.uid)
        .get()
        .then((value) {
      for (var element in value.docs) {
        _firestore.collection('friendly').doc(element.id).delete();
